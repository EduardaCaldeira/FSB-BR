...
class MXFaceDataset(Dataset):
    def __init__(self, root_dir, method, order, fraction, file_path, local_rank):
        ...
        if self.fraction<1 or self.method=='adaptive':
            self.protocol_pruned_dataset()
        
        if self.order=='id' or self.order=='global':
            with open('../../../../data/mcaldeir/output/labels/' + self.method + '/' + self.order + '/label_map_'+str(int(self.fraction*100))+'.pkl', 'rb') as fp:
    ...
    def __getitem__(self, index):
        ...
        #if int(label)==2643:
        #    print(self.label_map[int(label)])
        #elif int(label)<2643:
        #    print('error')
        ...
        #print(int(label))
        #print(self.label_map[int(label)])
        if self.method=='adaptive' or self.method is None:
