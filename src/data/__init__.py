...
def get_dataset(rank, **kwargs):
    trainset = FaceDataset(kwargs["dataset_path"], kwargs["model_name"], is_train=True)
    ...
    testset = [TestFaceDataset(path, kwargs["model_name"], is_train=False) for path in kwargs["test_dataset_path"]]
