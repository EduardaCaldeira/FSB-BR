...
def evaluate_mad_performance(prediction_scores, model_name, verbose=False):
    if model_name == "MADPromptS":
        thr_20 = -0.0607759058475494
        thr_10 = -0.00659613590687513
        thr_1 = 0.6734311581
    elif model_name == "MADation":
        thr_20 = -0.06028209627
        thr_10 = 0.1043157205
        thr_1 = 1.64875412

    bpcer_apcer20 = sum(prediction_scores<=thr_20)/len(prediction_scores)
    bpcer_apcer10 = sum(prediction_scores<=thr_10)/len(prediction_scores)
    bpcer_apcer1 = sum(prediction_scores<=thr_1)/len(prediction_scores)
