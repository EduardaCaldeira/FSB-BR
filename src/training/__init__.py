...
def get_trainer(rank, world_size, model_name, model, preprocess, trainset, dataloader, train_sampler, training_type, config, header, test_dataloader=None, test_sampler=None):
    if model_name == "MADPromptS" or model_name == "MADation":
        trainer = TrainerClip(rank, world_size, model, preprocess, trainset, dataloader, train_sampler, training_type, config, header, test_dataloader, test_sampler)
    else:
        raise ValueError()
