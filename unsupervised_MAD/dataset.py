...
class TestDataset(Dataset):
    ...
    def __init__(self, csv_file, input_shape=(224, 224)):
        ...
        self.dataframe = pd.read_csv(csv_file)
        '''
        self.composed_transformations = transforms.Compose(
        ...
        '''
        self.composed_transformations =albumentations.Compose([
            albumentations.SmallestMaxSize(max_size=input_shape[0], interpolation=cv2.INTER_AREA, p=1),
            albumentations.CenterCrop(height=input_shape[0], width=input_shape[0], p=1),
            albumentations.Normalize(PRE__MEAN, PRE__STD, p=1),
            ToTensorV2(),
        ])
    ...
    def __getitem__(self, idx):
        ...
        #image = self.composed_transformations(image)
        image = self.composed_transformations(image=image)['image']
