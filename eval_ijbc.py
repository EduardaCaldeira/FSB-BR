...
parser.add_argument('--general_path', type=str)
...
class Embedding(object):
    def __init__(self, prefix, data_shape, batch_size=1):
        ...
        if network == "mfn":
            ...
            backbone = iresnet100(num_features=512, use_se=False).to(f"cuda:{gpu_id}")
        elif network == "iresenet50":
            backbone= iresnet50(num_features=512, use_se=False).to(f"cuda:{gpu_id}")
        elif network == "ViT-L":
            backbone = get_model(args.network, dropout=0, fp16=False).to(f"cuda:{gpu_id}")
        elif network == "swin_t":
            backbone = build_model(SwinFaceCfg()).to(f"cuda:{gpu_id}")
        else:
            backbone = None
            exit()
        ...
        if network ==  "swin_t":
            backbone.backbone.load_state_dict(weight["state_dict_backbone"])
            backbone.fam.load_state_dict(weight["state_dict_fam"])
            backbone.tss.load_state_dict(weight["state_dict_tss"])
            backbone.om.load_state_dict(weight["state_dict_om"])
    ...
    def forward_db(self, batch_data):
        ...
        if self.network == "ViT-L":
            feat, _, _ = self.model(imgs)
        else:
...
def cossim_dist(scores, labels, save_png):
    labels_bol = labels.astype(bool)

    genuine_scores = np.array(scores[labels_bol])
    imposter_scores = np.array(scores[~labels_bol])

    gen_avg = np.mean(genuine_scores)
    imp_avg = np.mean(imposter_scores)

    # print statistics
    print('gen avg:', gen_avg)
    print('imp avg:', imp_avg)

    print(save_png.replace('.jpg', '.txt'))
    with open(save_png.replace('.jpg', '.txt'), 'w') as f:
        f.write('gen_avg: ' + str(gen_avg) + '\n')
        f.write('imp_avg: ' + str(imp_avg))
...
templates, medias = read_template_media_list(
    os.path.join('%s/meta' % general_path,
...
start = timeit.default_timer()
img_path = image_path
img_list_path = '%s/meta/%s_name_5pts_score_retina.txt' % (general_path, target.lower())
...
for method in methods:
    ...
    cossim_dist(scores[method], label, os.path.join(save_path, 'dist.jpg'))
